...
namespace JobScheduler.FunctionApp.Tests.IntegrationTests
{
    public class JobExecutorIntegrationTests : IDisposable
    {
        ...
        {
            // Test Bearer Auth
            await TestAuthType("bearer", "BEARER_TOKEN", "bearer-secret-123", (request) =>
            {
            ...
            // Test API Key Auth
            await TestAuthType("apikey", "API_KEY_TOKEN", "api-key-456", (request) =>
            {
            ...
            // Test No Auth
            await TestAuthType("none", null, null, (request) =>
            {
        ...
        private async Task TestAuthType(string authType, string? envVarName, string? tokenValue, Action<HttpRequestMessage> verifyAuth)
        {
            // Arrange
